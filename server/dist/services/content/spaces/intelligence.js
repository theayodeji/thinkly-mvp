...
export const generateChatResponse = async (spaceId, userId, message, _history = []) => {
    ...
    const stream = await geminiService.generateChatStream("Space Content: " + space.content + "\n\nSources:\n" + sourcesContext + "\n\n" + "History:\n" + historyText);
    // We return the stream, and a callback to save the full response once done
    return {
        stream,
        saveResponse: async (fullResponse) => {
            await ChatMessageModel.create({
                chatId: chat._id,
                spaceId,
                role: "assistant",
