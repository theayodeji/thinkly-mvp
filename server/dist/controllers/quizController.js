...
import { getQuizBySpaceIdAndUserId, evaluateQuizSubmission } from "../services/study/quizzes/evaluation.js";
export const generateQuiz = catchAsync(async (req, res, next) => {
    const savedQuiz = await generateAndSaveQuiz(req.params.spaceId, req.userId);
    res.status(201).json({ quiz: savedQuiz });
});
export const getQuiz = catchAsync(async (req, res, next) => {
    const quiz = await getQuizBySpaceIdAndUserId(req.params.spaceId, req.userId);
