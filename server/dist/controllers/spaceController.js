...
import { getSpacesByUserId, getSpaceByIdAndUserId, createNewSpace, deleteSpaceById, updateSpaceTitle } from "../services/content/spaces/management.js";
import { generateChatResponse } from "../services/content/spaces/intelligence.js";
import ChatModel from "../models/Chat.js";
import ChatMessageModel from "../models/ChatMessage.js";
export const getSpaces = catchAsync(async (req, res, next) => {
    const spaces = await getSpacesByUserId(req.userId);
    res.status(200).json({ spaces });
});
export const getSpace = catchAsync(async (req, res, next) => {
    const space = await getSpaceByIdAndUserId(req.params.id, req.userId);
    res.status(200).json({ space });
});
export const createSpace = catchAsync(async (req, res, next) => {
    const space = await createNewSpace(req.userId);
    res.status(200).json({ space });
});
export const deleteSpace = catchAsync(async (req, res, next) => {
    await deleteSpaceById(req.params.id, req.userId);
    res.status(200).json({ message: "Space deleted" });
});
export const updateSpace = catchAsync(async (req, res, next) => {
    await updateSpaceTitle(req.params.id, req.userId, req.body.title);
...
export const chat = catchAsync(async (req, res, next) => {
    ...
    const { stream, saveResponse } = await generateChatResponse(spaceId, req.userId, message, history);
...
export const getChatHistory = catchAsync(async (req, res, next) => {
    const chat = await ChatModel.findOne({ spaceId: req.params.id, userId: req.userId });
    if (!chat) {
